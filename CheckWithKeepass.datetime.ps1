...
#//Prerequisites that need to be installed before using the script

#//DataOntap module 9.8 or newer required
#//Poshkeepass module script uses 2.53 atm
#//Download and install KeePass + setup of database with secure Masterpassword this will need to be entered when running the script and it tries to fetch the passwords inside the keepass database. Also remove the rest of the default keepass entries.

#Check wordfile for more info on setup

#If its not possible to run ¨get¨ commands and ps is complaining on NuGet provider not working test below
#run command
#//[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

#then run the minimum required NuGet version PS want to use
...
# After installing keepass and setup a database (make a securepassword for this keepass also, will need to be entered when running the script) add the passwords how wever you want, in groups or wathever, but would advise to keep it simple.
# I have my listed in the root part of Keepass and each cluster/cluster password in its own entry.
...
#//Start bt opening Keepass and creating a database use advanced settings and set an 1.0 key (needed for module to work) after this the connection between the db and PS is made with the profile command
...
#Install-Module NetApp.ONTAP -Force -Verbose
...
#//The Import module can be left as is so they are properly loaded if the jumpserver/computer/powershell is restarted
